#!/usr/bin/env bash
#
# Drive the supervisor StateGraph (apps.agent.cores.supervisor) on one question from the CLI.
#
...
# sources get an LLM merge. So it needs: the wiki built (knowledge/wiki/index.json), the vLLM up,
...
# Resolve the default dataset's wiki dir (config-driven, e.g. knowledge/v0.1/wiki) and check it's built.
...
exec "$PY" -m apps.agent.cores.supervisor "$@"
