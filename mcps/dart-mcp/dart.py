...
REPORT_PATTERNS = {"연간": "FY", "3분기": "TQQ", "반기": "HYA", "1분기": "FQA"}  # 손익계산서는 TQQ

# 현금흐름표용 특별 패턴
CASH_FLOW_PATTERNS = {"연간": "FY", "3분기": "TQA", "반기": "HYA", "1분기": "FQA"}  # 현금흐름표는 TQA

# 재무상태표용 특별 패턴
BALANCE_SHEET_PATTERNS = {"연간": "FY", "3분기": "TQA", "반기": "HYA", "1분기": "FQA"}  # 재무상태표도 TQA
...
def format_numeric_value(value_text: str, decimals: str) -> str:
    ...
    Returns:
        포맷팅된 숫자 문자열
    """
    # XBRL fact values are already the full reported amount. The `decimals`
    # attribute is PRECISION metadata (e.g. -6 = accurate to the nearest
    # million), NOT a scale factor — it must not be used to rescale the value.
    # (Rescaling here inflated values by 10**-decimals and pushed them past
    # float64's exact-integer range, producing wrong, rounding-corrupted numbers.)
    s = value_text.replace(",", "").strip()
    try:
...
async def search_disclosure(
    ...
    try:
        ...
        await ctx.info(f"{len(disclosures)}개의 정기공시를 찾았습니다. XBRL 데이터 조회 및 분석을 시도합니다.")
        ...
        for disclosure in disclosures[:disclosure_count]:
            ...
            await ctx.info(f"공시 {processed_count}/{disclosure_count} 분석 중: {report_name} (접수번호: {rcept_no})")
            ...
            try:
                ...
                if requested_items:
                    is_relevant = any(
                        item in financial_data
                        and financial_data[item] not in INVALID_VALUE_INDICATORS
                        and not financial_data[item].startswith("오류(")
                        and not financial_data[item].startswith("분석 중")
        ...
        if len(disclosures) > disclosure_count:
            result += (
                f"※ 총 {len(disclosures)}개의 정기공시 중 최신 {disclosure_count}개에 대해 분석을 시도했습니다.\n"
            )
        ...
        for disclosure in disclosures[:disclosure_count]:
            try:
                ...
                if not xbrl_text.startswith(("DART API 오류:", "API 요청 실패:", "ZIP 파일", "<인코딩 오류:")):
                    ...
                    error_summary = xbrl_text.split("\n")[0][:100]
                    api_errors.append(f"{report_name}: {error_summary}")
                    await ctx.warning(f"XBRL 데이터 조회 오류 ({report_name}): {error_summary}")
            except Exception as e:
                ...
                try:
                    ...
                    try:
                        ...
                        if valid_items_count >= 1:
                            ...
                            for item, value in financial_data.items():
                                if (
                                    value not in INVALID_VALUE_INDICATORS
                                    and not value.startswith("오류(")
                                    and not value.startswith("분석 중")
                                ):
                                    result += f"| {item} | {value} |\n"
                            ...
                            await ctx.info(f"[{report_name}] {current_statement_type}의 유효한 데이터가 없습니다.")
                    except Exception as e:
                        await ctx.warning(f"XBRL 파싱/분석 중 오류 발생 ({report_name}): {e}")
...
async def search_business_information(
    ...
    try:
        ...
        await ctx.info(f"{len(disclosures)}개의 정기공시를 찾았습니다. 적절한 공시를 선택하여 정보를 추출합니다.")

        # 사업정보를 포함할 가능성이 높은 정기보고서를 우선순위에 따라 필터링
        priority_reports = ["사업보고서", "반기보고서", "분기보고서"]
        ...
        try:
            ...
            # 추출 결과 확인
            if section_text.startswith(f"공시서류 다운로드 실패") or section_text.startswith(
                f"'{information_type}' 섹션을 찾을 수 없습니다"
            ):
...
async def get_financial_json(
    corp_code: str, bsns_year: str, reprt_code: str, fs_div: str = "OFS"
) -> Tuple[List[Dict[str, Any]], Optional[str]]:
    """
...
def get_report_code_name(reprt_code: str) -> str:
    ...
    Returns:
        보고서 이름
    """
    code_to_name = {"11011": "사업보고서", "11012": "반기보고서", "11013": "1분기보고서", "11014": "3분기보고서"}
...
async def search_json_financial_data(
    ...
    try:
        ...
        if statement_type is None:
            await ctx.info(f"{company_name}의 {bsns_year}년 {report_name} {fs_div_name} 전체 재무정보를 검색합니다.")
        else:
...
def _run():
    ...
    # Newer mcp (>=~1.10) ships DNS-rebinding protection that rejects any
    # request whose Host header isn't an allowlisted host with 421 Misdirected
    # Request — which blocks remote clients (Host: <public-ip>). We serve over
    # the network on purpose and gate access with DART_MCP_TOKEN, so turn it off.
    # No-op on older mcp (e.g. 1.6.0) that lacks the field.
    try:
    ...
    token = os.environ.get("DART_MCP_TOKEN")
    if not token:
        print("WARNING: DART_MCP_TOKEN unset — SSE endpoint is UNAUTHENTICATED.", flush=True)
        mcp.run(transport="sse")
        return

    # Gate every request behind a shared bearer token.
    import uvicorn
    ...
    class _BearerAuth(BaseHTTPMiddleware):
        async def dispatch(self, request, call_next):
            # The SSE transport has two legs:
            #   GET  /sse        -> opens the stream (auth-gated here)
            #   POST /messages/  -> per-request channel, keyed by a 128-bit
            #                       session_id that is ONLY revealed over an
            #                       already-authenticated /sse stream (and that
            #                       FastMCP rejects if unknown).
            # We therefore gate only the stream-opening leg. This lets a
            # ?token=<token> on the /sse URL authenticate the whole session —
            # clients don't propagate query params to the /messages POST, but
            # they don't need to: that leg is protected by the session_id secret.
            if not request.url.path.startswith("/messages"):
                # Accept the standard header (Authorization: Bearer <token>) or a
                # ?token=<token> query param (for browsers / clients that can't
                # set headers). Header is preferred — query tokens leak into logs.
    ...
    print(f"DART MCP (SSE, token-gated) on http://{host}:{port}/sse", flush=True)
