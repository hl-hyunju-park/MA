...
_PROMPT_TMPL = (
    ...
    "    · **박스 목록(필수·완전)**: 페이지의 **모든 박스를 빠짐없이** 한 줄에 하나씩 나열하고, 각 박스 "
    "이름 뒤에 **범례 분류를 괄호로 반드시 표기** "
    "(예: 'CP LLC (회색=Incorporated)', 'Celadon Core LLC (흰색=To-Be-Set-up, ★=bank settlement)', 'X (하늘색=…)'). "
    "이 목록은 연결 목록과 **별개로 항상** 작성하며 생략 금지. "
    "또한 기본색(보통 회색)과 **다른 색/기호로 강조된 박스**(흰색=신설예정, 하늘색·★ 등 특수표시)는 "
    "'**특수표시 박스**:' 소제목 아래 **범례 분류별로 모아** 다시 한 번 나열하라 "
    "(예: '흰색(To-Be-Set-up): New Fund III GP, New CP Fund III L.P., …' / '★(bank settlement): …'). "
    "범례가 없으면 색상만이라도 표기.\n"
