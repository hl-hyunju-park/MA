...
- **`auto`** (default) — the **supervisor** (`apps/agent/agents/supervisor.py`): a LangGraph
...
in `core.py`/`agents/supervisor.py`, outside the compiled graph — so the full architecture is drawn
...
flowchart TD;
  ...
  subgraph SUP["supervisor — StateGraph, Command(goto) routing (agents/supervisor.py)"];
    direction TB;
  ...
  subgraph WIKI["wiki backend — LangGraph StateGraph (agents/wiki/build.py)"];
    direction TB;
