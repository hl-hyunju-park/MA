...
``src/stella_kb`` compiles the workbook into the vectorless wiki (``knowledge/wiki/``:
...
Layout:
    ...
    cores/        the agent backends — supervisor.py · dart.py · wiki/ (LangGraph)
    retrieval/    deterministic wiki access (lookup / open_page / trace_links) — no LLM
    utils/        datasets.py — dataset (wiki version) registry + cached WikiStore
